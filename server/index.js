...
const funx = require('./functions/funx');
...
async function instantiateSeleniumStaff() {
...
app.get('/api/selenium', (req, res) => {
  try {
    ...
    instantiateSeleniumStaff();

    const propties = {
      query: req.query.name + ' is amazing',
      driver,
    };
    const hours = funx.googleit(propties).catch(console.error);
...
wsServer.on('request', function (request) {
  ...
  connection.on('message', function (message) {
    ...
    if (message.type === 'utf8') {
      ...
      const userMessage = dataFromClient.caibUser + ' from server !!';
      ...
      if (dataFromClient.type === typesDef.USER_EVENT) {
        ...
        json.data = { users, userActivity, userMessage };
      } else if (dataFromClient.type === typesDef.CONTENT_CHANGE) {
        editorContent = dataFromClient.content;
        json.data = { editorContent, userActivity, userMessage };
      }
